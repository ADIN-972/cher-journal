...
export default function CustomStoryRequests() {
  const { stories, isLoading, error, fetchStories, cancelStory, clearError } =
    useCustomStoryStore();
  ...
  const translateGenre = (genre: string): string => {
    const genreTranslations: Record<string, string> = {
      PASSIONS_CHARNELLES: t("genres.PASSIONS_CHARNELLES.title"),
      ROMANCES_TENDRES: t("genres.ROMANCES_TENDRES.title"),
      MYSTERIES_SENSUELS: t("genres.MYSTERIES_SENSUELS.title"),
      INTERDITS: t("genres.INTERDITS.title"),
      CONQUETES: t("genres.CONQUETES.title"),
      REVES_SECRETS: t("genres.REVES_SECRETS.title"),
      PASSION_BRUTALE: t("genres.PASSION_BRUTALE.title"),
      AMOUR_COMPLIQUE: t("genres.AMOUR_COMPLIQUE.title"),
      DESIR_NOCTURNE: t("genres.DESIR_NOCTURNE.title"),
      LIBERATION: t("genres.LIBERATION.title"),
      DECOUVERTE_DE_SOI: t("genres.DECOUVERTE_DE_SOI.title"),
      INTIMITE_PSYCHOLOGIQUE: t("genres.INTIMITE_PSYCHOLOGIQUE.title"),
      EVEIL_DU_DESIR: t("genres.EVEIL_DU_DESIR.title"),
      RELATIONS_TRANSFORMATRICES: t("genres.RELATIONS_TRANSFORMATRICES.title"),
      MEMOIRE_DU_CORPS: t("genres.MEMOIRE_DU_CORPS.title"),
    };
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-1">
              Suivez l'état de vos demandes de création d'histoires
              personnalisées
            </p>
          </div>
          <a
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mt-3">
          {sortedStories.length} demande{sortedStories.length !== 1 ? "s" : ""}{" "}
          trouvée{sortedStories.length !== 1 ? "s" : ""}
        </p>
      </div>
      ...
      <div className="space-y-4 ">
        {sortedStories.map((story) => (
          <div
            key={story.id}
            className={`bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 hover:border-rose-300 dark:hover:border-rose-700 transition-colors overflow-hidden ${story.status === "REJECTED" ? "dashed bg-red-100/10" : "test"}`}>
            <div className="">
              <div className="flex items-start justify-between  p-6 bg-white/40">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4  text-sm p-6">
                <div>
